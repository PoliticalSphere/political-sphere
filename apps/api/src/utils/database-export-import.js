...
class DatabaseExportImportManager {
  ...
  async exportSchema(connection) {
    ...
    for (const table of tables) {
      const columns = connection.db.prepare(`PRAGMA table_info(${table.name})`).all();
      ...
      await withTransaction(
        async (_transaction, connection) => {
          ...
          for (const tableName of tablesToImport) {
            ...
            await this.importTableFromJSON(connection, tableName, tableData, clearExisting, dryRun);
  ...
  async exportToCSV(options = {}) {
    ...
    try {
      ...
      for (const tableName of tablesToExport) {
        const csvFile = await this.exportTableToCSV(connection, tableName, includeHeaders);
