...
function parseList(line) {
  ...
  if (bracket) {
    return bracket[1].split(',').map((s) => s.trim()).filter(Boolean);
  }
  return line
    .split('\n')
    .map((l) => l.trim())
...
function validateAssumptions(block, confidenceBlock) {
  const failures = [];
  if (!block) failures.push('Missing ASSUMPTIONS block with at least one bullet.');
  else {
    const items = block
      .split('\n')
      .map((l) => l.trim())
      .filter((l) => l.startsWith('- '));
    if (items.length === 0) failures.push('ASSUMPTIONS must include at least one list item.');
  }
  if (!confidenceBlock) failures.push('Missing CONFIDENCE block.');
  else {
    const self = confidenceBlock.match(/self_estimate:\s*([0-9.]+)/)?.[1];
    const areasLine = confidenceBlock.match(/high_risk_areas:\s*(.*)/)?.[1] ?? '';
    const val = self ? Number(self) : NaN;
    if (!(val >= 0 && val <= 1)) failures.push('CONFIDENCE.self_estimate must be in [0,1].');
    const areas = parseList(areasLine);
    if (!areas || areas.length === 0) failures.push('CONFIDENCE.high_risk_areas should include at least one area.');
  }
  return failures;
}
...
function validateOutput(block) {
  ...
  if (!type || type !== 'unified-diff') failures.push('OUTPUT.type must be "unified-diff".');
  const includes = parseList(includesLine);
  if (!includes.includes('tests')) failures.push('OUTPUT.includes must include "tests".');
  return failures;
}
...
(function main() {
  ...
  if (failures.length) {
    ...
    console.error(`PR header validation failed with ${failures.length} error(s).`);
    process.exit(1);
  }
