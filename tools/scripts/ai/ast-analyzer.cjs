...
class ASTAnalyzer {
	...
	analyzeComplexity(ast) {
		...
		metrics.halstead.volume =
			metrics.halstead.length * Math.log2(metrics.halstead.vocabulary || 1);
		metrics.halstead.difficulty = (n1 / 2) * (N2 / (n2 || 1));
		metrics.halstead.effort =
			metrics.halstead.volume * metrics.halstead.difficulty;
  ...
  extractSemanticTokens(ast) {
	...
	generateReport(analysis) {
		...
		report.push(
			`  Halstead Volume: ${analysis.complexity.halstead.volume.toFixed(2)}`,
		);
		report.push(
			`  Halstead Difficulty: ${analysis.complexity.halstead.difficulty.toFixed(2)}\n`,
		);
...
if (require.main === module) {
	...
	if (analysis.error) {
		...
		if (analysis.location) {
			console.error(
				`   at line ${analysis.location.line}, column ${analysis.location.column}`,
			);
		}
		process.exit(1);
	}
