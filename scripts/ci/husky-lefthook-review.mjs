	...
	async reviewLefthookConfig() {
		...
		try {
			...
			if (!config["pre-commit"]) {
				this.addIssue(
					"error",
					"pre-commit section is missing from .lefthook.yml",
				);
			}
			...
			if (!config["commit-msg"]) {
				this.addIssue(
					"warning",
					"commit-msg section is missing from .lefthook.yml",
				);
			}
			...
			if (config["pre-commit"] && config["pre-commit"].parallel !== true) {
				this.addIssue(
					"suggestion",
					"Consider enabling parallel execution for pre-commit hooks",
				);
			}
			...
			if (config["pre-commit"] && config["pre-commit"].commands) {
				Object.entries(config["pre-commit"].commands).forEach(([name, cmd]) => {
					if (!cmd.run) {
						this.addIssue(
							"error",
							`Command '${name}' has no run configuration`,
							lefthookPath,
						);
					}
				});
	...
	async reviewHuskyScripts() {
		...
		requiredHooks.forEach((hook) => {
			if (!existingHooks.includes(hook)) {
				this.addIssue(
					"warning",
					`Required hook '${hook}' is missing from .husky`,
				);
			}
		});
		...
		existingHooks.forEach((hook) => {
			try {
				...
				if (!content.includes("set -e") && !content.includes("set -eu")) {
					this.addIssue(
						"warning",
						`Hook '${hook}' lacks proper error handling`,
						hookPath,
					);
				}
			} catch (error) {
	...
	async reviewIntegration() {
		...
		if (fs.existsSync(packageJsonPath)) {
			...
			if (!packageJson.scripts || !packageJson.scripts.prepare) {
				this.addIssue(
					"warning",
					"package.json prepare script not configured for lefthook",
				);
			} else if (!packageJson.scripts.prepare.includes("lefthook")) {
				this.addIssue(
					"warning",
					"package.json prepare script does not install lefthook",
				);
			}
		}
	...
	async reviewPerformance() {
		...
		if (fs.existsSync(lefthookPath)) {
			...
			if (!content.includes("parallel: true")) {
				this.addIssue(
					"suggestion",
					"Enable parallel execution to improve performance",
				);
			}
			...
			if (commandMatches && commandMatches.length > 10) {
				this.addIssue(
					"warning",
					"High number of commands may impact performance",
				);
			}
		}
	}
