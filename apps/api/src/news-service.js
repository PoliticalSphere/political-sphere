...
function sanitizeTagsInput(tags) {
	...
	if (tags.length > MAX_TAGS) {
		throw createValidationError(
			`Too many tags. Maximum allowed: ${MAX_TAGS}`,
			"tags",
		);
	}
	const sanitized = [];
...
function sanitizeSourcesInput(sources) {
	...
	if (sources.length > MAX_SOURCES) {
		throw createValidationError(
			`Too many sources. Maximum allowed: ${MAX_SOURCES}`,
			"sources",
		);
	}
	...
	for (const rawSource of sources) {
		...
		if (!ALLOWED_SOURCE_PROTOCOLS.includes(protocol)) {
			if (
				!(
					protocol === "http" &&
					LOCALHOST_SOURCE_NAMES.includes(parsed.hostname)
				)
			) {
				throw createValidationError(
...
class NewsService {
	...
	async _writeItems(items) {
		...
		if (typeof this.store.write === "function")
	...
	async create(payload = {}) {
		...
		const title = sanitizeHtml(
			validateTextField("Title", payload.title, 1, 200),
		);
		const excerpt = sanitizeHtml(
			validateTextField("Excerpt", payload.excerpt, 1, 1000),
		);
		const content = sanitizeHtml(
			validateTextField("Content", payload.content, 1, 20000),
		);
	...
	async list(opts = {}) {
		...
		if (tag) {
			...
			items = items.filter(
				(i) => Array.isArray(i.tags) && i.tags.includes(validatedTag),
			);
		}
