...
class ExpertKnowledge {
  ...
  initializeKnowledge() {
    return {
      patterns: {
        errorHandling: {
          express: "Use async error wrapper middleware for clean error handling",
          promises: "Always use try-catch with async/await or .catch() with promises",
          validation: "Validate input with Zod schemas before processing",
        },
        testing: {
          unitTests: "Test pure functions in isolation with mocks for dependencies",
          integration: "Test API endpoints with supertest, include auth headers",
        ...
        performance: {
          ...
          async: "Use Promise.all for parallel operations, avoid sequential awaits",
          bundling: "Code-split large bundles, lazy load non-critical code",
        },
        architecture: {
          separation: "Domain logic in services, HTTP in routes, validation in middleware",
      ...
      solutions: {
        commonErrors: {
          ...
          EACCES: "Permission denied - check file permissions or use sudo carefully",
          ECONNREFUSED: "Connection refused - ensure service is running on that port",
        },
        testFailures: {
