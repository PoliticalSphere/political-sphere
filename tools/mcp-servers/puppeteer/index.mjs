  ...
  async callTool(request) {
    ...
    switch (name) {
      case "puppeteer_fetch_dom": {
        ...
        const waitUntil = typeof args.waitUntil === "string" ? args.waitUntil : "domcontentloaded";
        const timeout =
      ...
      case "puppeteer_screenshot": {
        ...
        const base64 = await withBrowser(async (browser) => {
          ...
          if (args.viewport && typeof args.viewport === "object") {
            const width = typeof args.viewport.width === "number" ? args.viewport.width : 1280;
            const height = typeof args.viewport.height === "number" ? args.viewport.height : 720;
        ...
        return {
          content: [
            {
              type: "text",
              text: JSON.stringify({ url, image: base64, encoding: "base64" }, null, 2),
            },
          ],
        };
      }
