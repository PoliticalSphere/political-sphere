...
const PATTERNS_FILE = resolve(
	__dirname,
	"../../../ai/ai-learning/patterns.json",
);
...
function assessCompetence() {
	...
	const qualityPassRate =
		metrics.qualityGatesPassed + metrics.qualityGatesFailed > 0
			? metrics.qualityGatesPassed /
				(metrics.qualityGatesPassed + metrics.qualityGatesFailed)
			: 0;
	...
	if (avgResponseTime > 2000)
		recommendations.push(
			"Optimize response times - consider caching or pre-computation",
		);
	if (cacheHitRate < 0.7)
		recommendations.push("Improve cache hit rate - review caching strategy");
	if (qualityPassRate < 0.9)
		recommendations.push(
			"Address quality gate failures - review common issues",
		);
	if (avgSatisfaction < 0.7)
		recommendations.push(
			"Improve user satisfaction - gather feedback on suggestions",
		);
	if (avgThroughput < 5)
		recommendations.push(
			"Increase task throughput - optimize parallel processing",
		);
	...
	if (patterns.patterns && patterns.patterns.length > 0) {
		...
		if (successRate < 0.8)
			recommendations.push(
				"Review recent patterns - identify areas for improvement",
			);
	}

	// Context-awareness specific recommendations (tests assert on these phrases).
	if (
		metrics.contextRecallAccuracy !== undefined &&
		metrics.contextRecallAccuracy < 0.9
	) {
	...
	if (
		metrics.patternUsageCorrectness !== undefined &&
		metrics.patternUsageCorrectness < 0.95
	) {
		recommendations.push("Review pattern usage");
	}
	if (
		metrics.modelDriftResistance !== undefined &&
		metrics.modelDriftResistance < 0.9
	) {
