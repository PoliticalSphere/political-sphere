...
describe("Path Security Library Tests", () => {
	describe("validateFilename", () => {
		it("should accept valid filenames", () => {
			...
			assert.strictEqual(
				validateFilename("test-file_123.json"),
				"test-file_123.json",
			);
		...
		it("should reject filenames with invalid characters", () => {
			assert.throws(
				() => validateFilename("file<script>.txt"),
				/invalid characters/,
			);
			assert.throws(
				() => validateFilename("file;rm-rf.txt"),
				/invalid characters/,
			);
		});
	...
	describe("validateRelativePath", () => {
		it("should accept valid relative paths", () => {
			...
			assert.strictEqual(
				validateRelativePath("src/index.js"),
		...
		it("should reject parent directory references", () => {
			...
			assert.throws(
				() => validateRelativePath("sub/../../escape"),
				/parent directory/,
			);
		});
	...
	describe("safeJoin", () => {
		...
		it("should reject paths that escape base directory", () => {
			assert.throws(
				() => safeJoin("/base", "../escape"),
				/path separators|outside base/,
			);
		});
	...
	describe("validateTrustedPath", () => {
		...
		it("should reject parent directory references", () => {
			assert.throws(
				() => validateTrustedPath("../etc/passwd"),
				/parent directory/,
			);
			assert.throws(
				() => validateTrustedPath("sub/../../../escape", "/base"),
				/parent directory/,
			);
		});
	...
	describe("isPathWithinBase", () => {
		...
		it("should return false for paths outside base", () => {
			...
			assert.strictEqual(
				isPathWithinBase("/base", "/base-other/file.txt"),
				false,
			);
		});
