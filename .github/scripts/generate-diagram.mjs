...
async function generateMermaidDiagram() {
	...
	function createNodeId(path, name) {
		...
		const prefix =
			parts.length > 0 ? parts[parts.length - 1].substring(0, 3) : "root";
	...
	async function processDirectory(dirPath, parentId, indent = "    ") {
		...
		for (const folder of items.folders) {
			...
			else if (folder === "documentation")
				folderColor = FOLDER_COLORS.documentation;
			else if (folder === "ISSUE_TEMPLATE")
				folderColor = FOLDER_COLORS.issueTemplates;
			else if (folder === "PULL_REQUEST_TEMPLATE")
				folderColor = FOLDER_COLORS.prTemplates;
		...
		for (const file of items.files) {
			...
			else if (dirPath.includes("documentation"))
				fileColor = FILE_COLORS.documentation;
			else if (dirPath.includes("ISSUE_TEMPLATE"))
				fileColor = FILE_COLORS.template;
			else if (dirPath.includes("PULL_REQUEST_TEMPLATE"))
				fileColor = FILE_COLORS.prTemplate;
			else if (file === "copilot-instructions.md")
				fileColor = FILE_COLORS.copilot;
