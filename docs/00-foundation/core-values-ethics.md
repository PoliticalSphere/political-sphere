...
```mermaid
graph TD
    A[Identify Ethical Issue] --> B{Does it align with core values?}
    B -->|Yes| C[Proceed with implementation]
    B -->|No| D[Conduct ethical analysis]
    D --> E{Impact assessment}
    E --> F{High risk?}
    F -->|Yes| G[Ethics Committee review]
    F -->|No| H[Team discussion & resolution]
    G --> I[Committee decision]
    H --> J[Document decision]
    I --> J
    J --> K[Implement with safeguards]
```
