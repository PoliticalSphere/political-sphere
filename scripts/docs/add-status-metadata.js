...
function hasMetadataTable(content) {
  return content.includes('Classification') && 
...
function addStatusToMetadataTable(content, status) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('Classification') && line.includes('Version') && line.includes('Last Updated')) {
      tableHeaderIndex = i;
      inTable = true;
    } else if (inTable && line.match(/^\s*\|[\s:|-]+\|[\s:|-]+\|/)) {
      tableSeparatorIndex = i;
    } else if (inTable && tableSeparatorIndex > -1 && line.includes('|') && !line.match(/^[\s|:-]+$/)) {
      tableDataIndex = i;
      break;
    }
  }
