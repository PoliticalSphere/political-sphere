...
if (!hasTestRunner) {
  if (fileExists(path.join(nmBin, "vitest")) || fileExists(path.join(nmBin, "vitest.cmd")))
    hasTestRunner = true;
  if (fileExists(path.join(nmBin, "jest")) || fileExists(path.join(nmBin, "jest.cmd")))
    hasTestRunner = true;
}
...
if (!hasTestRunner)
  missing.push("Vitest or Jest (declared in package.json or installed in node_modules)");

if (missing.length > 0) {
  console.error("\nERROR: Missing required tools:");
  for (const m of missing) console.error(` - ${m}`);
  console.error("\nRemediation:");
  console.error(" - Ensure Node, Git and npx are installed in the environment.");
  console.error(
