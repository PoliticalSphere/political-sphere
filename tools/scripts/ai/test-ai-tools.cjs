...
try {
  ...
  if (complexity.cyclomatic > 1 && complexity.maxNesting >= 2) {
    console.log(`  ✅ PASS: Complexity analysis working (cyclomatic: ${complexity.cyclomatic}, nesting: ${complexity.maxNesting})\n`);
    passed++;
  } else {
...
try {
  ...
  if (fs.existsSync(testFile)) {
    ...
    if (!analysis.error && analysis.complexity && analysis.tokens && analysis.patterns) {
      console.log(`  ✅ PASS: Full analysis complete (complexity: ${analysis.complexity.cyclomatic}, tokens: ${analysis.tokens.length})\n`);
      passed++;
    } else {
