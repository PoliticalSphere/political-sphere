...
async function fastPathFromCache() {
	...
	try {
		...
		try {
			const rawControls = await fsp
				.readFile(CONTROLS_FILE, "utf8")
				.catch(() => null);
			if (rawControls) {
...
function analyzeFile(filePath) {
	// Security: Validate path from git output to prevent traversal
	const validatedPath = validateTrustedPath(filePath, ROOT);

	if (!fs.existsSync(validatedPath)) return null;
	...
	const functions = (
		content.match(
			/\bfunction\s+\w+\s*\(|\bconst\s+\w+\s*=\s*\(?\s*\w*\s*=>/g,
		) || []
	).length;
	...
	for (const block of functionBlocks) {
		...
		if (ln > 200)
			largeFunctions.push({ lines: ln, preview: block.slice(0, 200) });
	}
...
function summarizeFindings(changedFiles) {
	...
	for (const a of analyses) {
		...
		if (a.todos > 0)
			fileFindings.push({
				severity: "medium",
				message: `${a.todos} TODO/FIXME/HACK comments`,
			});
		if (a.consoleLogs > 0)
			fileFindings.push({
				severity: "low",
				message: `${a.consoleLogs} console.* calls present`,
			});
