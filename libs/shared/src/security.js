...
function resolveSecret(envKey, fallback) {
	...
	if (process.env.NODE_ENV === "production") {
		throw new Error(
			`${envKey} must be set to a secure value in production environments`,
		);
	}
	return fallback;
}
...
export function createSecurityHeaders(options = {}) {
	...
	directives.set(
		"frame-ancestors",
		new Set(frameAncestors.length ? frameAncestors : ["'none'"]),
	);
	directives.set(
		"form-action",
		new Set(formAction.length ? formAction : ["'self'"]),
	);
	...
	const frameOptions = frameAncestors.includes("'none'")
		? "DENY"
		: "SAMEORIGIN";
